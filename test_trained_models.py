...
def create_sample_images():
    ...
    # Create a simple pattern for host image
    host_img = np.zeros((224, 224, 3), dtype=np.uint8)
    host_img[:, :, 0] = 128  # Red channel
    host_img[::20, :, 1] = 200  # Green lines
    host_img[:, ::20, 2] = 200  # Blue columns
    
    # Create a different pattern for secret image
    secret_img = np.zeros((224, 224, 3), dtype=np.uint8)
    secret_img[:, :, 1] = 128  # Green channel
    # Add a pattern that will be preserved when hidden
    for i in range(0, 224, 40):
        for j in range(0, 224, 40):
            secret_img[i:i+20, j:j+20, 0] = 200  # Red squares
            secret_img[i:i+20, j:j+20, 2] = 200  # Blue squares
    
    # Save the images
    host_pil = Image.fromarray(host_img)
    secret_pil = Image.fromarray(secret_img)
    
    host_pil.save('my_images/host_sample.png')
    secret_pil.save('my_images/secret_sample.png')
    
    print("Sample images created in my_images directory")


def main():
    """
    Main function to run the trained models on test images.
    """
    ...
    # Look for images in the my_images directory with proper extension checking
    image_dir = "my_images"
    ...
    if len(image_paths) < 2:
        print("Not enough images found in my_images directory.")
        return
    
    # Process the first two images as a demo
    if len(image_paths) >= 2:
        host_path = image_paths[0]
        secret_path = image_paths[1]
        ...
        # Print results
        print("\n" + "="*60)
        print("STEGANOGRAPHY RESULTS (224x224)")
        print("="*60)
        
        if 'dwt' in results:
            print(f"\nDWT Model Results:")
            print(f"  Hiding PSNR: {results['dwt']['hiding_psnr']:.2f} dB")
            print(f"  Recovery PSNR: {results['dwt']['recovery_psnr']:.2f} dB")
            print(f"  Hiding SSIM: {results['dwt']['hiding_ssim']:.4f}")
            print(f"  Recovery SSIM: {results['dwt']['recovery_ssim']:.4f}")
        else:
            print("\nDWT Model: Not available")
            
        if 'ilwt' in results:
            print(f"\nILWT Model Results:")
            print(f"  Hiding PSNR: {results['ilwt']['hiding_psnr']:.2f} dB")
            print(f"  Recovery PSNR: {results['ilwt']['recovery_psnr']:.2f} dB")
            print(f"  Hiding SSIM: {results['ilwt']['hiding_ssim']:.4f}")
            print(f"  Recovery SSIM: {results['ilwt']['recovery_ssim']:.4f}")
        else:
            print("\nILWT Model: Not available")
        
        # Create visualization
        visualize_results(host_tensor, secret_tensor, results)
        ...
        if 'dwt' in results and 'ilwt' in results:
            dwt_hid = results['dwt']['hiding_psnr']
            ilwt_hid = results['ilwt']['hiding_psnr']
            dwt_rec = results['dwt']['recovery_psnr']
            ilwt_rec = results['ilwt']['recovery_psnr']
