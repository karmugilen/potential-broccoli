...
def train_model(model, model_name, dataset, num_epochs=100):
    ...
    learning_rate = 1e-4  # Higher initial learning rate for faster convergence
    ...
    # Create optimizer with a more aggressive scheduler for better dynamic learning rate
    ...
    return epoch_losses, epoch_hiding_psnrs, epoch_recovery_psnrs, best_hiding_psnr, best_recovery_psnr
    ...
    for i, idx in enumerate(test_indices):
        ...
        with torch.no_grad():
            try:
                ...
                fig.suptitle(f'{model_name} - Sample {i+1} - Hiding PSNR: {hiding_psnr.item():.2f} dB, Recovery PSNR: {recovery_psnr.item():.2f} dB')
...
def main():
    ...
    # Initialize models
    dwt_model = StarINNWithDWT(channels=6, num_blocks=num_blocks, hidden_channels=hidden_channels)
    ilwt_model = StarINNWithILWT(channels=6, num_blocks=num_blocks, hidden_channels=hidden_channels)
    
    # Train DWT model
    dwt_losses, dwt_hiding_psnrs, dwt_recovery_psnrs, dwt_best_hid, dwt_best_rec = train_model(
        dwt_model, "DWT", dataset, num_epochs=num_epochs
    )
    
    # Train ILWT model
    ilwt_losses, ilwt_hiding_psnrs, ilwt_recovery_psnrs, ilwt_best_hid, ilwt_best_rec = train_model(
    ...
    # Test both models
    dwt_avg_hid, dwt_avg_rec = test_model(dwt_model, dataset, "DWT", num_samples=5)
    ilwt_avg_hid, ilwt_avg_rec = test_model(ilwt_model, dataset, "ILWT", num_samples=5)
    ...
    hiding_winner = "DWT" if ilwt_best_hid < dwt_best_hid else "ILWT"  # Note: ILWT has higher values in our test
    recovery_winner = "DWT" if dwt_best_rec > ilwt_best_rec else "ILWT"
    
    print(f"\nWinner - Best Hiding PSNR: {hiding_winner}")
    print(f"Winner - Best Recovery PSNR: {recovery_winner}")
